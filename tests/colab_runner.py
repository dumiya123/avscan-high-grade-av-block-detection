...
DRIVE_ZIP = "/content/drive/MyDrive/AtrionNet_Implementation.zip"
WORK_DIR  = "/content/AtrionNet_Implementation"
...
if os.path.exists(DRIVE_ZIP):
    ...
    GITHUB_URL = "https://github.com/YOUR_USERNAME/AtrionNet_Implementation.git"
    subprocess.run(["git", "clone", GITHUB_URL, WORK_DIR], check=True)
    print(f"✅ Cloned from GitHub to {WORK_DIR}")

os.chdir(WORK_DIR)
print(f"📂 Working directory: {os.getcwd()}")

# ════════════════  CELL 5 — Verify LUDB Data  ═════════════════════════════════
data_dir = Path(WORK_DIR) / "ml_component" / "data" / "raw" / "ludb"
...
if len(hea_files) >= 10:
    ...
    try:
        dl_script = Path(WORK_DIR) / "ml_component" / "download_data.py"
        if dl_script.exists():
            subprocess.run([sys.executable, str(dl_script)],
                           cwd=str(Path(WORK_DIR) / "ml_component"), check=True)
...
weights_candidates = [
    Path(WORK_DIR) / "ml_component" / "outputs" / "weights" / "atrion_hybrid_best.pth",
    Path(WORK_DIR) / "ml_component" / "weights" / "atrion_hybrid_best.pth",
    Path(WORK_DIR) / "ml_component" / "weights" / "AtrionNet_H1_best.pth",
]
ATRION_WEIGHTS = None
...
if ATRION_WEIGHTS is None:
    print("❌ AtrionNet weights NOT found. Check ml_component/outputs/weights/ or ml_component/weights/")

# ════════════════  CELL 7 — Add Project to Python Path  ══════════════════════
ML_ROOT = str(Path(WORK_DIR) / "ml_component")
if ML_ROOT not in sys.path:
    sys.path.insert(0, ML_ROOT)
print(f"✅ sys.path includes {ML_ROOT}")
