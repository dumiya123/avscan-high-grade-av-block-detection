...
def plot_confusion_matrix(tp, fp, fn, save_path):
    """Generates detection confusion matrix. TN is N/A for instance detection."""
    plt.figure(figsize=(8, 6))
    
    labels = np.array([[f"{tp}", f"{fp}"], [f"{fn}", "N/A"]])
    data = np.array([[tp, fp], [fn, 0]]) # numeric for mapping colors
